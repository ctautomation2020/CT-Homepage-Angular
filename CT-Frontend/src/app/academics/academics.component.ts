...
export class AcademicsComponent implements OnInit {
    ...
    getSemester(count:number){
        this.sems = this.acadService.getSemesters(this.academicType,this.currentRegulation.regu_type,count);
    }
    updateSemester(sem:string, title:string){ 
        ...
        if(sem != 'pe') this.subjects = this.acadService.getSubjects(this.academicType,sem,this.currentRegulation.regu_type);
        this.getElectives(sem, this.currentRegulation.regu_type);
    }
    setSubjectPDF(subcode:string, title:string, type?:string, track?:string){
        let pdfLink:string;
        
        if(type) pdfLink = this.acadService.getPath(this.academicType,this.currentRegulation.regu_type, this.currentElective);
        else pdfLink = this.acadService.getPath(this.academicType,this.currentRegulation.regu_type, this.currentSem.sem_code);
    ...
    getElectives(sem:string, regu:string){
        if(this.acadService.isElectiveAvailable(this.academicType,sem,regu)){
            this.electives = this.acadService.getElectiveSubjectsList(this.academicType,regu,sem);
        }
        else this.electives = [];
    }
    showElectives(type:string, track:string){
        ...
        this.electiveSubs = this.acadService.getElectives(this.academicType,this.currentRegulation.regu_type,type,track);
