...
export class StudentComponent implements OnInit {
    ...
    getYears(start_year:number = 2017){
        let years:number[] = [];
        for(let year = this.studentService.current_year; year >= start_year; year--) years.push(year);
        return years;
    }
    getBatchYear(year:number){
        return (this.student_type.type == 'be') ? (year + " - " + (year + 4)) : (year + " - " + (year + 2));
    }
    updateYear(year:number){
        this.year = year;
    }
    getStudents(){
        this.router.navigateByUrl("/people/student/"+ this.student_type.type + "/" +this.year);
    }
    ngOnInit(): void {
